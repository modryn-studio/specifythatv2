...
function detectProjectType(description: string): ProjectTypeResult {
...
function generateProjectName(description: string, projectType: string): string {
...
function generateConstraints(projectType: string): string[] {
  ...
  const typeConstraints: { [key: string]: string[] } = {
...
function generateFeatures(projectType: string): string[] {
...
function generateNonGoals(projectType: string): string[] {
  const nonGoals: { [key: string]: string[] } = {
    ecommerce: [
...
function generateDataEntities(projectType: string): string[] {
...
function generateRelationships(projectType: string): string[] {
...
function generateUserFlow(projectType: string): string[] {
...
function generateAPIs(projectType: string, description: string): string[] {
...
function generateBuildSequence(features: string[]): string[] {
  return [
...
export function generateAllAnswers(projectDescription: string): Answers {
  const description = projectDescription.trim();
  const { primary: projectType } = detectProjectType(description);
  const audience = extractTargetAudience(description);
  const projectName = generateProjectName(description, projectType);
  const features = generateFeatures(projectType);
  const entities = generateDataEntities(projectType);
  ...
  return {
    ...
    4: generateConstraints(projectType).map(c => `- ${c}`).join('\n'),
    5: deployment,
    6: features.map(f => `- ${f}`).join('\n'),
    7: generateNonGoals(projectType).map(n => `- ${n}`).join('\n'),
    8: entities.map(e => `- **${e}**: Core entity for ${e.toLowerCase()} data`).join('\n'),
    9: generateRelationships(projectType).map(r => `- ${r}`).join('\n'),
    10: generateUserFlow(projectType).map((step, i) => `${i + 1}. ${step}`).join('\n'),
    11: generateAPIs(projectType, description).map(a => `- ${a}`).join('\n'),
    12: generateBuildSequence(features).join('\n'),
    13: generateSuccessCriteria(projectType).map(c => `- [ ] ${c}`).join('\n')
  };
}
...
export function generateSpecMarkdown(projectName: string, answers: Answers): string {
